...
// Герои для подробных карточек, героя дня, тир-листа, драфта
const heroData = {
    slark: {
        id: "slark",
        name: "Slark",
...
const demoThreads = [
    {
        ...
        tag: "meta",
        preview:
            "Обсуждаем, когда Slark выглядит стабильно, а когда превращается в героя «по ситуации»...",
        text:
            "Slark сейчас очень сильный керри, но он чувствителен к контролю и вижену. Делитесь опытом: в каких драфтах вы готовы брать его на ранних стадиях, а где ждёте последних пиков? Какие герои/предметы контрят больше всего?",
        author: "MetaSeeker",
        replies: 9,
    ...
    },
    {
        ...
        tag: "builds",
        preview:
            "Echo Sabre, Kaya & Sange, BKB — или есть более жадные варианты сборки?",
        text:
            "Классическая сборка через Echo Sabre + Kaya & Sange всё ещё кажется лучшей, но иногда хочется поэкспериментировать. Кто-то пробовал более глас-канон билд через Dagon/Hex? Насколько это оправдано, особенно на высоком рейтинге?",
        author: "VoidEnjoyer",
        replies: 5,
    ...
    },
    {
        ...
        tag: "heroes",
        preview:
            "Поделитесь историями, как эти саппорты выигрывали вам игры в соло.",
        text:
            "В 7.40c WD, Clock и Lich ощущаются как очень сильные саппорты: хороший контроль, сильная линия и полезные ульты. Расскажите, какие билды, шард/агоним тайминги и фишки вы используете, чтобы выигрывать игры даже с рандом керри.",
        author: "SupportMain",
        replies: 11,
...
function setupMobileSidebar() {
    ...
    toggleBtn.addEventListener("click", () => {
        sidebar.classList.toggle("td-sidebar--open");
    });
...
function setupHeroOfDay() {
    ...
    const randomId =
        heroOfDayPool[Math.floor(Math.random() * heroOfDayPool.length)];
...
function setupHeroModal() {
    ...
    closeBtn.addEventListener("click", () =>
        backdrop.classList.remove("td-modal-backdrop--visible")
    );
    ...
    // Кнопки «Подробнее» на карточках героев
    ...
    roleButtons.forEach((btn) => {
        btn.addEventListener("click", () => {
            ...
            roleButtons.forEach((b) =>
                b.classList.remove("td-chip-filter--active")
            );
...
function setupBuildVotes() {
    ...
    counts.forEach((span) => {
        ...
        if (storedVotes[id] != null) {
            span.textContent = String(storedVotes[id]);
        }
    });
    ...
    buildLikeButtons.forEach((btn) => {
        btn.addEventListener("click", (e) => {
            ...
            const countSpan = card.querySelector(
                ".td-vote-count[data-build-id='" + id + "']"
            );
            if (!countSpan) return;
...
function getDefaultTierState() {
    return {
        pool: heroIds.slice(), // все герои в пуле
        S: [],
        A: [],
        B: [],
        C: []
...
function loadTierState() {
    try {
        ...
        // Подстраховка: если нет каких-то героев/полей
        const state = getDefaultTierState();
        ["pool", "S", "A", "B", "C"].forEach((tier) => {
            if (Array.isArray(parsed[tier])) {
                state[tier] = parsed[tier].filter((id) => heroData[id]);
            }
        });
        // Удаляем дубли
        const allIdsUsed = new Set(
            [...state.pool, ...state.S, ...state.A, ...state.B, ...state.C]
        );
        heroIds.forEach((id) => {
            if (!allIdsUsed.has(id)) state.pool.push(id);
        });
        return state;
    } catch {
...
function renderTierlist(state) {
    const tiers = ["pool", "S", "A", "B", "C"];
    tiers.forEach((tier) => {
        const container = document.querySelector(
            tier === "pool" ? "#tierPool" : `#tier${tier}`
        );
        if (!container) return;
        container.innerHTML = "";
...
function setupTierlist() {
    ...
    function onDragOver(e) {
        ...
        dropZone.classList.add("td-tier-droppable--over");
    ...
    function onDrop(e) {
        ...
        saveTierState(state);
        renderTierlist(state);
        attachTierDragHandlers(); // заново навешиваем drag
        draggedId = null;
    }

    function attachTierDragHandlers() {
        $$(".td-tier-item").forEach((item) => {
...
function renderDraft(state) {
    ...
    heroIds.forEach((id) => {
        ...
        // Функция создания маленького «чипа»
        const makeChip = (clickHandler, showRole) => {
            const chip = document.createElement("div");
            chip.className = "td-draft-hero";
            chip.dataset.heroId = id;
            chip.innerHTML = `<span>${hero.name}</span>${
                showRole
                    ? `<span class="td-draft-hero-role">${hero.roles.join("/")}</span>`
                    : ""
            }`;
        ...
        if (status === "pool") {
            ...
            slot
                .querySelector(".td-draft-slot-remove")
            ...
            // также чип в пуле для переключения в enemy
            poolEl.appendChild(
                makeChip(() => {
                    state[id] = "enemy";
                    saveDraftState(state);
...
function renderDraftHints(state) {
    ...
    if (our.length === 0 && enemy.length === 0) {
        ...
        if (roleCount.support === 0) {
            hints.push("У твоей команды нет саппортов — подумай добавить хотя бы одного героя поддержки.");
        }
        if (roleCount.carry >= 3) {
            hints.push("Много керри в одной команде — может не хватить контроля и вижена.");
        }
        if (roleCount.mid === 0) {
            hints.push("Нет ярко выраженного мидера — это может ударить по темпу игры.");
        }
    }

    if (enemy.length >= 3 && our.length >= 1) {
        hints.push(
            "У врагов достаточно героев — подумай про сейв/контроль, чтобы не дать им навязать свои драки."
        );
    }
...
function loadThreadSubscriptions() {
    try {
        threadSubscriptions = JSON.parse(
            localStorage.getItem("tdThreadSubs") || "[]"
        );
...
function renderThreads(threads) {
    ...
    if (!threads.length) {
        listEl.innerHTML =
            '<div class="td-card"><p class="td-small-note">Пока нет тем. Создай первую.</p></div>';
        return;
    }
    ...
    threads.forEach((thread) => {
        ...
        threadEl.addEventListener("click", () => {
            ...
            if (expanded) {
                bodyEl.textContent = thread.text;
            } else {
                bodyEl.textContent = thread.preview;
            }
        });
        ...
        subBtn.addEventListener("click", (e) => {
            ...
            if (idx === -1) {
                threadSubscriptions.push(id);
            } else {
                threadSubscriptions.splice(idx, 1);
            }
...
function setupNewThreadModal() {
    ...
    const openModal = () =>
        modalBackdrop.classList.add("td-modal-backdrop--visible");
    const closeModal = () =>
        modalBackdrop.classList.remove("td-modal-backdrop--visible");
...
function updateProfileUI(profile) {
    ...
    if (sidebarStatusEl) sidebarStatusEl.textContent = "Локальный профиль";
}
...
function setupRegisterForm() {
    ...
    form.addEventListener("submit", async (e) => {
        ...
        statusEl.className = "td-form-status td-form-status--visible";
        ...
        try {
            // КЛЮЧЕВОЙ МОМЕНТ: URL с параметром action
            ...
            if (response.ok && !data.error) {
                statusEl.textContent = "Успешно зарегистрировано!";
                statusEl.className = "td-form-status td-form-status--success";
                localStorage.setItem("tdProfile", JSON.stringify(profileData));
                if (typeof updateProfileUI === "function") updateProfileUI(profileData);
                form.reset();
            } else {
                statusEl.textContent = "Ошибка: " + (data.error || "Неизвестная ошибка");
                statusEl.className = "td-form-status td-form-status--error";
            }
        } catch (err) {
